...
const ProblemsTable = ({ problems, user }: { problems: ProblemWithSolved[]; user: DbUser }) => {
	...
	const handleDelete = async (problemId: string) => {
		try {
			const { deleteProblem } = await import("@/modules/problems/actions");
		...
		<div className="w-full max-w-7xl mx-auto space-y-8 p-6">
			...
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				<div>
					...
					<p className="text-muted-foreground">Manage and solve coding problems</p>
				</div>
				<Button
			...
			{totalPages > 1 && (
				<div className="flex items-center justify-between">
					<p className="text-sm text-muted-foreground">
						Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, filteredProblems.length)} of{" "}
