...
import type { Submission } from "@/src/generated/browser";
...
export const SubmissionHistory = ({ submissions = [] }: SubmissionHistoryProps) => {
	...
	const formatMemory = (memory: string | null): string => {
		...
		try {
			...
			const avgMemory = memoryArray.reduce((a: number, b: any) => a + parseFloat(b), 0) / memoryArray.length;
	...
	const formatTime = (time: string | null): string => {
		...
		try {
			...
			const avgTime = timeArray.map((t: any) => parseFloat(t.replace(" s", ""))).reduce((a: number, b: number) => a + b, 0) / timeArray.length;
