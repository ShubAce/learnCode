...
const ProfileStats = ({ submissions, solvedCount, playlistCount }: ProfileStatsProps) => {
	const acceptedSubmissions = submissions.filter((s: Submission) => s.status === "Accepted").length;
	const wrongAnswers = submissions.filter((s: Submission) => s.status === "Wrong Answer").length;
